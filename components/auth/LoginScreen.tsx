...
const LoginScreen: React.FC = () => {
    ...
    const handleSignUp = async (e: React.FormEvent) => {
        ...
        setUiLoading(true);
        try {
            // Passa o nome e telefone como metadados para o Trigger usar.
            const { error } = await supabase.auth.signUp({
                email, password, options: { data: { name, phone, role: 'client' } }
            });
            if (error) throw error;
        ...
        if (view === 'signup') {
            return (
                <form onSubmit={handleSignUp} className="space-y-4">
                    ...
                    <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="Telefone (opcional)" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm" />
...
}; // <-- ESTA CHAVE FALTANTE PODE TER CAUSADO O ERRO
